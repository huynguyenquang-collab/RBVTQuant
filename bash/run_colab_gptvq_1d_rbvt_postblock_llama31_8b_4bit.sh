...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/gptvq_1d_rbvt_postblock_llama31_8b_4bit}"
...
echo "=== GPTVQ-1D vs GPTVQ-1D+RBVT post-block | Llama-3.1-8B | 4-bit ==="
echo "Model: $MODEL"
...
echo "RBVT placement: post_block | lambda=$RBVT_LAMBDA | topk=$RBVT_TOPK"
echo "M-step: disabled so post-block RBVT is not overwritten"
...
from quantizers import apply_rbvt  # noqa: F401

print("GPTVQ + RBVT import smoke check passed.")
PY
