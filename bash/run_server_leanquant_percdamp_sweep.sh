...
# LeanQuant percdamp sweep for RTN only on a Linux GPU server.
# Runs percdamp {0.01, 0.15} for 4-bit and 3-bit, then aggregates PPL/lm-eval.
...
PERCDAMP_VALUES="${PERCDAMP_VALUES:-0.01 0.15}"
SWEEP_OUTPUT_ROOT="${SWEEP_OUTPUT_ROOT:-$ROOT_DIR/outputs/leanquant_percdamp_sweep}"
...
LOG_FILE="$LOG_DIR/leanquant_percdamp_sweep_${TIMESTAMP}.log"
first_run=1

{
  echo "=== LeanQuant RTN percdamp sweep ==="
  echo "Percdamp values: $PERCDAMP_VALUES"
  echo "Bits: 4 3"
  echo "Method: RTN only"
  echo "Exponent: ${LEANQUANT_EXPONENT:-4.0}"
  echo "Sweep output: $SWEEP_OUTPUT_ROOT"
} | tee -a "$LOG_FILE"

for percdamp in $PERCDAMP_VALUES; do
  tag="$(percdamp_tag "$percdamp")"
  run_output="$SWEEP_OUTPUT_ROOT/runs/percdamp_${tag}"
  # LeanQuant cache metadata includes percdamp, so each percdamp keeps an
  ...
  {
    echo
    echo "=== Percdamp $percdamp ==="
    BITS="4 3" \
    METHODS="rtn" \
    LEANQUANT_PERCDAMP="$percdamp" \
...
rows = []
pattern = re.compile(r"percdamp_(.+)$")
for path in sorted((root / "runs").glob("percdamp_*/benchmark_results.csv")):
    match = pattern.search(path.parent.name)
    percdamp = match.group(1).replace("p", ".") if match else ""
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            if row.get("codebook") == "LeanQuant" and row.get("method") == "RTN":
                row = {"leanquant-percdamp": percdamp, **row}
                rows.append(row)

if not rows:
    raise SystemExit(f"No LeanQuant RTN results found under {root / 'runs'}")

rows.sort(key=lambda row: (float(row["leanquant-percdamp"]), int(row["bits"])))
fieldnames = ["leanquant-percdamp"] + [
    name for name in rows[0] if name != "leanquant-percdamp"
]
...
print("\nLeanQuant RTN percdamp sweep results")
