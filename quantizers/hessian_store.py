"""Disk-backed cache for LeanQuant Hessian matrices."""
...
class HessianStore:
    ...
    def initialize(self, metadata: dict):
        if self._manifest.get("layers") and self.metadata != metadata:
            raise ValueError(
                f"Hessian cache metadata mismatch at {self.root}: "
    ...
    def get(self, layer_name: str) -> torch.Tensor | None:
        ...
        if not path.exists():
            raise FileNotFoundError(f"Missing cached Hessian for {layer_name!r}: {path}")
        try:
